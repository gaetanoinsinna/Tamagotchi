...
{					
	...
	if(cuddle == 0)
	/*START CONVERSION per il volume*/
	ADC_start_conversion();	


	
	
if(isButtonPressed == 0 && isDead == 0)
if(getDisplayPoint(&display, Read_Ads7846(), &matrix )){
		if(display.x >= 50 && display.x <= 190 && display.y >=90 && display.y<230)
		{
			if(happiness < MAX_HAPPINESS && satiety != 0 && cuddle == 0){
				
				//happiness++;
				
			LCD_DrawRectangle(50+5*11,90+5*12,5,5,Red,Red);
			LCD_DrawRectangle(50+5*16,90+5*12,5,5,Red,Red);
			LCD_DrawRectangle(50+5*13,90+5*13,10,5,Red,Red);
				
			for(i = 0; i < HEART_HEIGHT; i++){
				for(j = 0; j < HEART_LENGHT; j++){
					if(heart[i][j]==1) LCD_DrawRectangle(15+1*j,100+1*i,1,1,Red,Red);
		...
		for(i = 0; i < HEART_HEIGHT; i++){
			for(j = 0; j < HEART_LENGHT; j++){
				...
				/*IMPOSTA LA MODALITA' CUDDLE SE LA FELICITA' E' PIU' GRANDE DI 1 OPPURE SE MANCANO MENO DI 2 SECONDI*/
				if((happiness > 1 || happinessSeconds < 3) && (satiety > 1 || satietySeconds < 3)){
				cuddle = 1;
				sound = 0;
					isButtonPressed = 1;
	...
	if((LPC_GPIO1->FIOPIN & (1<<25)) == 0 ){
		...
		if(isSelected == 0) isSelected++;
		else if(isSelected>=1) isSelected = 0;
		isSelected = 1;
		sound = 0;
